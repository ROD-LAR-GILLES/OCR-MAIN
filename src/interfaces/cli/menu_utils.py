...
def detect_pdf_type_automatically(pdf_path: Path) -> Tuple[SystemConfig, str]:
    """Detecta automáticamente el tipo de PDF y sugiere configuración óptima."""
    try:
        ...
        if file_size_mb > 10:
            # Archivo grande, probablemente escaneado
            config.engine_type = ENGINE_TYPE_OPENCV
            config.dpi = 300
            config.confidence_threshold = 75.0
            config.enable_deskewing = True
            config.enable_denoising = True
            detection_info = f"Documento escaneado detectado ({file_size_mb:.1f} MB) - Usando motor OpenCV"
        else:
            # Archivo pequeño, probablemente nativo
            config.engine_type = ENGINE_TYPE_BASIC
            config.dpi = 300
            config.confidence_threshold = 60.0
            detection_info = f"Documento nativo detectado ({file_size_mb:.1f} MB) - Usando motor básico"
        
        config.language = DEFAULT_LANGUAGE
        logger.info(f"Detección automática: {detection_info}")
        
        return config, detection_info
        
    except Exception as e:
        logger.error(f"Error en detección automática: {str(e)}")
        # Fallback a configuración básica
        config = SystemConfig()
        return config, "Error en detección - Usando configuración por defecto"
